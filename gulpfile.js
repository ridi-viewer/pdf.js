...
var doGeneric = function doGeneric(onlyForViewer) {
  console.log();
  console.log('### Creating generic viewer' + (!onlyForViewer ? ' with PDF.js' : ''));
  ...
  if (!onlyForViewer) {
    tasks.unshift(createBundle(defines).pipe(gulp.dest(GENERIC_DIR + 'build')));
  }
  return merge(tasks);
};
...
gulp.task('generic', gulp.series('buildnumber', 'locale', function () {
  return doGeneric(false);
}));

gulp.task('generic-viewer', gulp.series('buildnumber', 'locale', function () {
  return doGeneric(true);
}));
...
gulp.task('lint-viewer', function (done) {
  doLint(done, true);
});
...
gulp.task('dist-pre', gulp.series('generic', 'components', 'lib', 'minified', function() {
  var VERSION = getVersionJSON().version;
